...
let parse_from_function com f file p =
	let t = Common.timer "parsing" in
	Lexer.init file true;
	incr stats.s_files_parsed;
	let data = (try Parser.parse com (f()) with e -> t(); raise e) in
	t();
...
let parse_file com file p =
	...
	let data = parse_from_function com (fun () -> Lexing.from_channel ch) file p in
	close_in ch;
	data
