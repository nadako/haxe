	...
	(try
		...
		*)
			...
			let rec expr_s w e =
				...
				(match e.eexpr with
					...
					| TCall ({ eexpr = TIdent "__is__" }, [ expr; { eexpr = TTypeExpr(md) } ] ) ->
						...
						write w (md_s md);
						write w " )"
					| TCall ({ eexpr = TIdent "__is__" }, expr :: typedexpr :: _ ) ->
						write w "( ";
						expr_s w expr;
						write w " is ";
						write w (t_s typedexpr.etype);
						write w " )";
		...
		let on_method_closure_generated cl cf field =
			...
			begin
				let t_equals = TFun ([("obj",false,t_dynamic)],com.basic.tbool) in
				...
				let v_obj = alloc_var "obj" t_dynamic in
				let e_obj = mk_local v_obj pos in
				let e_as = mk (TIdent "__as__") t_dynamic pos in
				let e_cast = mk (TCall (e_as,[e_obj;e_obj])) t_closure pos in
				let v_obj_casted = alloc_var "c" t_closure in
				let e_obj_casted = mk_local v_obj_casted pos in
				let e_cond = mk (TBinop (OpEq,e_obj_casted,ExprBuilder.make_null e_obj_casted.etype pos)) com.basic.tbool pos in
				let e_if_null = mk (TIf (e_cond, (mk_return (ExprBuilder.make_bool com false pos)), None)) com.basic.tvoid pos in
				let e_check =
					mk (TBinop (OpEq,
						e_this_obj,
				...
				let e_equals = mk (TBlock [
					mk (TVar (v_obj_casted, Some e_cast)) com.basic.tvoid pos;
					e_if_null;
					mk_return e_check;
			...
			begin
				...
				let cf_hashcode = mk_class_field "GetHashCode" t_hashcode true pos (Method MethNormal) [] in
				cl.cl_ordered_fields <- cf_hashcode :: cl.cl_ordered_fields;
				cl.cl_fields <- PMap.add cf_hashcode.cf_name cf_hashcode cl.cl_fields;

				let e_obj_hashcode = mk (TCall ((mk_field_access gen e_this_obj "GetHashCode" pos),[])) com.basic.tint pos in
				let e_field = ExprBuilder.make_string com field pos in
				let e_field_hashcode = mk (TCall ((mk_field_access gen e_field "GetHashCode" pos),[])) com.basic.tint pos in
		...
		let on_static_method_closure_generated cl cl_orig field =
			...
			begin
				...
				let e_check = mk (TCall (e_is,[e_obj; null t_closure pos])) com.basic.tbool pos in
			...
			begin
				...
				let e_type_hashcode = mk (TCall ((mk_field_access gen e_typeof "GetHashCode" pos),[])) com.basic.tint pos in
				let e_field = ExprBuilder.make_string com field pos in
				let e_field_hashcode = mk (TCall ((mk_field_access gen e_field "GetHashCode" pos),[])) com.basic.tint pos in
