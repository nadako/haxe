...
and expr_def =
	...
	| EOptArray of expr * expr
	| EOptField of expr * string
...
let map_expr loop (e,p) =
	...
	| EOptArray (e1,e2) ->
		...
		let e2 = loop e2 in
		EOptArray (e1,e2)
	| EOptField (e,f) -> EOptField (loop e, f)
	) in
	(e,p)
...
let iter_expr loop (e,p) =
	...
	| EParenthesis e1 | EOptField(e1,_) | EField(e1,_) | EUnop(_,_,e1) | EReturn(Some e1) | EThrow e1 | EMeta(_,e1)
	| ECheckType(e1,_) | EDisplay(e1,_) | ECast(e1,_) | EUntyped e1 -> loop e1;
	| EOptArray(e1,e2) | EArray(e1,e2) | EBinop(_,e1,e2) | EFor(e1,e2) | EWhile(e1,e2,_) | EIf(e1,e2,None) -> loop e1; loop e2;
...
module Printer = struct
	let rec s_expr_inner tabs (e,_) =
		...
		| EOptArray (e1,e2) -> s_expr_inner tabs e1 ^ "?[" ^ s_expr_inner tabs e2 ^ "]"
		| EOptField (e,f) -> s_expr_inner tabs e ^ "?." ^ f
